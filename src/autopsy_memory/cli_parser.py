...
def build_parser(
    ...
    *,
    ...
    shared_server_parser.add_argument("--kind", help="Memory kind for shared-server check-memory, restore --check-policy, or restore-version --check-policy.")
    ...
    shared_server_parser.add_argument("--check-policy", action="store_true", help="With shared-server publish, restore, restore-version, relate, or link, dry-run the repo policy first and send the returned fingerprint/version as a stale-write guard.")
    shared_server_parser.add_argument("--expected-policy-fingerprint", help="With shared-server publish, restore, restore-version, relate, or link, reject the write unless the effective repo policy fingerprint still matches this value.")
    shared_server_parser.add_argument("--expected-policy-version-ns", type=int, help="With shared-server publish, restore, restore-version, relate, or link, reject the write unless the effective repo policy version_ns still matches this value.")
    ...
    shared_server_parser.add_argument("--allowed-memory-kind", action="append", help="With shared-server update-policy, allowed shared memory kinds. Repeat or comma-separate values.")
    shared_server_parser.add_argument("--clear-memory-kinds", action="store_true", help="With shared-server update-policy, clear the allowed memory kind allow-list.")
    ...
    shared_server_parser.add_argument("--allow-memory-writes", action="store_true", help="With shared-server update-policy, allow shared memory publish and restore-version writes.")
    shared_server_parser.add_argument("--disable-memory-writes", action="store_true", help="With shared-server update-policy, disable shared memory publish and restore-version writes.")
    ...
    shared_server_parser.add_argument("--last-hours", type=float, help="With shared-server audit, audit-integrity, or team-status, request audit events from the last N hours.")
    shared_server_parser.add_argument("--since", default="", help="With shared-server audit, audit-integrity, or team-status, include audit events at or after this ISO-8601 timestamp.")
    ...
    shared_server_parser.add_argument("--idempotency-key", help="With retry-safe shared-server mutations, reuse this client request key for safe manual retry. Generated automatically when omitted. Not used for token or invitation issuance because those return one-time plaintext tokens.")
