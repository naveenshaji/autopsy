...
def rerank_candidates(query: str, candidates: list[dict], config: dict | None) -> list[dict]:
    ...
    ranked_shortlist = sorted(
        ...
        key=lambda item: (
            ...
            item.get('activity_at') or item.get('updated_at') or '',
            item.get('stable_key') or '',
        ),
    )
    if len(candidates) <= candidate_limit:
        return ranked_shortlist
    return ranked_shortlist + candidates[candidate_limit:]
...
def handle_memory_graph_note_create(payload: dict) -> dict:
    ...
    return run_falkor_operation(
        falkor,
        lambda graph: falkor['module'].create_graph_note_payload(
            graph,
            tool=tool,
            workspace=workspace,
...
def handle_memory_graph_item_update(payload: dict) -> dict:
    ...
    return run_falkor_operation(
        falkor,
        lambda graph: falkor['module'].update_graph_item_payload(
            graph,
            tool=tool,
            workspace=workspace,
            stable_key=str(request.get('stable_key') or ''),
