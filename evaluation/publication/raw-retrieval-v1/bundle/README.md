...
**Publication status:** all six aggregate rows passed the same-clean-commit release gate at `a22c3c7b9946a7fa8a1cdb2c6dd0f50d9b58e71f`. See `MANIFEST.json` and `SHA256SUMS` for the source and sanitized artifact bindings.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| LoCoMo | Autopsy hybrid (package metadata 0.1.30) | 0.6030 | 0.4928 | 0.5435 | 0.3933 | 0.0000 | 600.92 ms | `aggregate/locomo-autopsy.json` |
| LoCoMo | built-in BM25 control | 0.5965 | 0.4974 | 0.5403 | 0.3866 | 0.0000 | 0.73 ms | `aggregate/locomo-builtin-bm25.json` |
| LoCoMo | Mem0 OSS 2.0.11 raw (`infer=False`) | 0.5254 | 0.4094 | 0.4588 | 0.2828 | 0.0000 | 12.08 ms | `aggregate/locomo-mem0-oss-raw.json` |
| LongMemEval-S upstream | Autopsy hybrid (package metadata 0.1.30) | 0.6301 | 0.3819 | 0.5035 | 0.6005 | 0.2143 | 126.22 ms | `aggregate/longmemeval-s-autopsy.json` |
| LongMemEval-S upstream | built-in BM25 control | 0.9690 | 0.8974 | 0.9374 | 0.8710 | 0.0000 | 0.10 ms | `aggregate/longmemeval-s-builtin-bm25.json` |
| LongMemEval-S upstream | Mem0 OSS 2.0.11 raw (`infer=False`) | 0.9499 | 0.8831 | 0.9200 | 0.8422 | 0.0000 | 6.79 ms | `aggregate/longmemeval-s-mem0-oss-raw.json` |
<!-- RESULTS_TABLE_END -->
...
> commit `a22c3c7b9946a7fa8a1cdb2c6dd0f50d9b58e71f`, Autopsy (package metadata `0.1.30`) achieved `0.6030 Recall-any@10` and
> `0.3933 MRR@10` on LoCoMo, and `0.6301 Recall-any@10` and
> `0.6005 MRR@50` on the LongMemEval-S upstream user-only
