...
- The running menu bar app silently keeps the resident Autopsy worker warm, so local context graph URLs stay available without needing agents to rerun `context-graph-url` just to revive the server.
