...
autopsy shared-server link graph-note:local shared:key --repo-scope /path/to/repo --relation references --fact-rating 0.9 --check-policy
...
`team-status` includes active, active never-used, active stale over 30 days,
last-used, expired, and revoked scoped-token counts,
...
keys. Stale repo-policy update/reset conflicts are summarized by count,
operation mode, rejection reason, and latest timestamp. Stale shared-memory
write/archive/restore conflicts are summarized by count, operation mode,
rejection reason, memory kind, archived-state counts, and latest timestamp.
`revoke-token` first uses the global-admin endpoint and
...
Add `--last-hours <n>` for a relative window, or `--since <iso-8601>` and
`--until <iso-8601>` for an explicit bounded audit window from servers that
advertise `audit_event_time_windows`.
...
audit events. Sensitive shared-server read surfaces such as users, token
metadata, scoped invite-token inventory, grants, access checks, audit reads,
memory lists/history, and shared-relation lists are also audited with compact
counts, limits, roles/reasons, statuses, and filter-presence flags. New
shared-server audit events also include hash-chain integrity fields and
...
bridge. Add `--check-policy` to dry-run the repo policy immediately before the
write and send the returned policy fingerprint/version as a stale-write guard.
Use manual `--expected-policy-fingerprint` and `--expected-policy-version-ns`
from `shared-server check-relation` only when reusing a previously reviewed
dry-run. `shared-server personal-links --repo-scope <repo>` lists only your own
...
`--check-policy` to dry-run the repo policy immediately before the write and
send the returned policy fingerprint/version as a stale-write guard. Use manual
`--expected-policy-fingerprint` and `--expected-policy-version-ns` from
`shared-server check-relation` only when reusing a previously reviewed dry-run.
