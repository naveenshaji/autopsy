...
class AutopsyCLIContractTests(unittest.TestCase):
    ...
    def test_shared_server_parser_accepts_team_access_controls(self):
        ...
        restore_plan_snapshot_args = parser.parse_args([
            "shared-server",
        ...
        unrelate_args = parser.parse_args(["shared-server", "unrelate", "rel_1", "--repo-scope", "repo-a"])
        ...
        self.assertEqual(restore_apply_snapshot_args.idempotency_key, "manual-restore-apply-key-1")
        ...
        self.assertEqual(unrelate_args.shared_server_action, "unrelate")
        self.assertEqual(unrelate_args.stable_key, "rel_1")
    ...
    def test_shared_server_audit_path_scopes_graph_repo_and_limit(self):
        ...
        csv_filtered_path = cli.shared_server_audit_path(
            "autopsy",
            "repo-a",
            limit=25,
            actions=["read_shared_context,read_personal_context"],
    ...
    def test_shared_server_shared_relation_paths_are_graph_scoped(self):
        self.assertEqual(
            cli.shared_server_relations_path(
                "autopsy",
                "repo-a",
                limit=25,
                source_key="shared:1",
            ...
            "/v1/shared-graphs/autopsy/relations?repo=repo-a&limit=25&source_key=shared%3A1&target_key=shared%3A2",
        )
        self.assertEqual(
    ...
    def test_shared_server_shared_relations_lists_graph_relations(self):
        parser = cli.build_parser()
        args = parser.parse_args([
            "shared-server",
            "shared-relations",
        ...
        def fake_request(_config, path, *, method="GET", payload=None, timeout=5.0):
            calls.append((path, method, payload))
            return {"items": [{"id": "rel_1", "source_key": "shared:1"}]}
        ...
        self.assertEqual(json.loads(stream.getvalue())["items"][0]["id"], "rel_1")
        self.assertEqual(
            calls,
            [
                (
    ...
    def test_shared_server_relate_posts_shared_relation_payload(self):
        parser = cli.build_parser()
        args = parser.parse_args([
            "shared-server",
            "relate",
        ...
        self.assertEqual(json.loads(stream.getvalue())["related"]["id"], "rel_1")
        self.assertEqual(
            calls,
            [
                (
    ...
    def test_shared_server_unrelate_posts_shared_relation_revoke(self):
        parser = cli.build_parser()
        args = parser.parse_args(["shared-server", "unrelate", "rel_1", "--repo-scope", "repo-a"])
        config = {"base_url": "https://shared.example", "graph_slug": "autopsy", "token": "secret"}
        calls: list[tuple[str, str, dict[str, str] | None]] = []

        def fake_request(_config, path, *, method="GET", payload=None, timeout=5.0):
            calls.append((path, method, payload))
            return {"id": "rel_1", "source_key": "shared:1"}
        ...
        self.assertEqual(json.loads(stream.getvalue())["id"], "rel_1")
        self.assertEqual(
            calls,
            [
                (
    ...
    def test_shared_server_restore_plan_snapshot_action_posts_snapshot_file(self):
        parser = cli.build_parser()
        snapshot = {
            "service": "autopsy-server",
            "schema_version": 1,
        ...
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as handle:
            ...
            snapshot_path = handle.name
        self.addCleanup(lambda: Path(snapshot_path).unlink(missing_ok=True))
        args = parser.parse_args(["shared-server", "restore-plan-snapshot", "--snapshot-file", snapshot_path, "--sample-limit", "9"])

        def fake_request(_config, path, **kwargs):
            self.assertEqual(path, "/v1/admin/export-snapshot/restore-plan?sample_limit=9")
            self.assertEqual(kwargs.get("method"), "POST")
            self.assertEqual(kwargs.get("payload"), snapshot)
            self.assertEqual(kwargs.get("timeout"), 30)
            return {
                ...
                "dry_run": True,
                "mode": "restore_plan",
                "totals": {"would_create": 1, "would_update": 0, "invalid_identities": 0},
                "restore_readiness": {"safe_to_apply": True},
                "audit": {"action": "plan_export_snapshot_restore"},
            }
        ...
        self.assertTrue(payload["ok"])
        self.assertTrue(payload["dry_run"])
        self.assertEqual(payload["mode"], "restore_plan")
    ...
    def test_shared_server_restore_apply_snapshot_action_posts_snapshot_file(self):
        ...
        def fake_request(_config, path, **kwargs):
            ...
            self.assertEqual(kwargs.get("extra_headers"), {"Idempotency-Key": "manual-restore-apply-key-2"})
            return {
                "ok": True,
                "dry_run": False,
                "mode": "restore_apply",
    ...
    def test_shared_server_restore_apply_snapshot_auto_idempotency_retries_transient(self):
        parser = cli.build_parser()
        snapshot = {
            "service": "autopsy-server",
            "schema_version": 1,
        ...
        self.assertEqual(calls[0]["path"], f"/v1/admin/export-snapshot/restore-apply?expected_plan_digest={expected_digest}&sample_limit=9")
    ...
    def test_shared_server_admin_tokens_action_prints_remote_payload(self):
        ...
        self.assertEqual(payload["items"][0]["token_fingerprint"], "abcdef1234567890")
    ...
    def test_shared_server_policy_reads_repo_policy(self):
        parser = cli.build_parser()
        args = parser.parse_args(["shared-server", "policy", "--repo-scope", "repo-a"])
        config = {"base_url": "https://shared.example", "graph_slug": "autopsy", "token": "secret"}
        calls: list[str] = []

        def fake_request(_config, path, **_kwargs):
            calls.append(path)
            return {"graph_slug": "autopsy", "repo": "repo-a", "allowed_relation_labels": []}
    ...
    def test_shared_server_team_status_summarizes_remote_access_and_scoped_tokens(self):
        ...
        self.assertEqual(payload["team"]["active_owner_grants_count"], 1)
